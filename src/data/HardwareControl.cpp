...
namespace LRI::RCI {
    ...
    namespace HWCTRL {
        namespace {
            ...
            RCP_LibInitData cbks{
                .sendData = nullptr,
                .readData = nullptr,
                .processTestUpdate = nullptr,
                .processBoolData = nullptr,
                .processSimpleActuatorData = nullptr,
                .processPromptInput = nullptr,
                .processTargetLog = nullptr,
                .processOneFloat = nullptr,
                .processTwoFloat = nullptr,
                .processThreeFloat = nullptr,
                .processFourFloat = nullptr,
            };
            ...
            std::set<HardwareQualifier> hwconfig;
        } // namespace

        int POLLS_PER_UPDATE;
        int PACKETS_POLLED_IN_LAST_FRAME;

        const std::vector<Error>& getErrors() { return errors; }

        void start(RCP_Interface* _interf, std::set<HardwareQualifier> quals) {
            if(interf) {
                return;
            }
            ...
            hasStarted = true;
            log = EventLog();
            hwconfig = std::move(quals);
            for(const auto& qual : hwconfig) log.createDevice(qual);
        }
        ...
        void requestRefresh(const HardwareQualifier& qual) {
            RCP_Error res = RCP_requestGeneralRead(qual.devclass, qual.id);
            log.addReadReq(qual);
