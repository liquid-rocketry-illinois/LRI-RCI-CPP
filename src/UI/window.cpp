...
// Functions and variables relating to the window and the borderless-ness
namespace {
    ...
    LRESULT borderlessCallback(HWND hwnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
        ...
        case WM_NCHITTEST: {
            ...
            case RegionClient:
            default:
                if(static_cast<float>(cursor.y - wpos.top) < CAPHEIGHT) return HTCAPTION;
                break;
            }
    ...
    GLFWwindow* setupBorderlessWindow() {
...
namespace {
    ...
    void renderBackgroundElements() {
        // initialization of function static variables is performed the first time this block is entered
        static const std::string VERSION_STRING =
            "RCI " + std::string(RCI_VERSION, RCI_VERSION_END) + "\nRCP " + std::string(RCP_VERSION, RCP_VERSION_END);
        static const float VER_STRING_HEIGHT = ImGui::CalcTextSize(VERSION_STRING.c_str()).y / style::getScaleFactor();

        const ImVec2 basePos = ImGui::GetMainViewport()->Pos;
        const ImVec2 blankSpace = style::getWindowSize() - ImVec2{0, 40_sc};
    ...
    void frame() {
        ...
        style::setScaleFactor(ImGui::GetMainViewport()->DpiScale);
